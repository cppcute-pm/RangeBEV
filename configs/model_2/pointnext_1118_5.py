...
dataset_cfgs=dict(
    ...
    use_true_neighbors_mask=True,
    true_neighbour_dist=25.0,
    use_coords_information=True,
)
...
loss_cfgs = dict(
    loss_type='G2M_triplet_huber_angle',
    triplet_loss_cfgs=dict(margin=0.2,
                            normalize_embeddings=True,
                            hard_mining=True,
                            pair_dist_info=False),
    huber_loss_dist_cfgs=dict(beta=0.01, # to tune
                            reduction='mean',
                            lamda=15), # to tune
    huber_loss_angle_cfgs=dict(angle_loss_type='cos_sim', 
                                angle_type=5,
                                in_features=256,
                               out_features=2,
                               feat_angle_dist_scale=1.0,
                               pair_type='all'), # to tune
    triplet_loss_weight=0.5,
    huber_loss_dist_weight=0.0,
    huber_loss_angle_weight=0.5,
