...
model_cfgs=dict(
    ...
    cmvpr_cfgs=dict(
        ...
        phase18_local_correspondence=False,
        ...
        aggregate_clusterly_and_semantically='only_clusterly',
        cluster_aggregator_type='GeM',
        cluster_aggregator_cfgs=dict(
            p=3,
            eps=1e-6,
            ),
        semantic_aggregator_type='GeM',
        semantic_aggregator_cfgs=dict(
            p=3,
            eps=1e-6,
            ),
        use_ordinary_aggregator=False,
        aggregate_and_match=True,
        aggregate_cluster_attn_type='attn_all',
        cluster_transformer_cfgs=dict(out_norm=True,
                                      attn_type='attn_all',
                                      d_model=256,
                                      n_head=8,
                                      num_layers=3),
        semantic_transformer_cfgs=dict(out_norm=True,
                                      d_model=256,
                                      n_head=8,
                                      num_layers=3),

        min_pc_dbscan_cluster_num_pt=4,
        min_img_ccl_cluster_num_pt=4,
        overlap_matrix_fuse_type='min',
        cluster_topk=2500,
        semantic_fuse_type='semantic_corresponded',
        cluster_correspondence_type='before',
        semantic_correspondence_type='before',
...
loss_cfgs=dict(
    ...
    global_loss_weight=0.25 * 10,
    cluster_loss_weight=0.25,
    semantic_in_batch_loss_weight=0.0,
    semantic_all_loss_weight=0.0,
)
...
# dgcnn_path=None
# resnet_path=None
# cmvpr_path='thfzy0mh/epoch_67.pth'
cmvpr_path=None
pretrained_cfgs=dict(
    cmvprnet=dict(
        image_path=resnet_path,
        image_cfgs=dict(
            backbone_part=0,  # 0 or 3 or 4
        ),
        image_aggregator_cfgs=dict(
            aggregate_part=0,
        ),
        pc_path=dgcnn_path,
        pc_cfgs=dict(
            backbone_part=2 # 2 or 1
        ),
        pc_aggregator_cfgs=dict(
            aggregate_part=0,
        ),
        cmvpr_path=cmvpr_path,
        # cmvpr_pretrained_aggregator=True,
        # phase_aggregator_cfgs=dict(
        #     aggregate_part=0,
        #     ),
    )
)
