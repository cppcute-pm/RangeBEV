...
FastTransformer_sparseops_path = "../FastPointTransformer-master/src/cuda_ops/functions"
import sys
sys.path.append(FastTransformer_sparseops_path)
import sparse_ops as ops
sys.path.remove(FastTransformer_sparseops_path)
# import cuda_sparse_ops as ops
...
class LocalSelfAttentionBase(nn.Module):
    ...
    def key_query_indices_from_key_query_map(self, kq_map):
        ...
        kq_indices[0] = torch.div(kq_indices[0], self.kernel_volume, rounding_mode='trunc')
        return kq_indices
...
class FastPointTransformer(nn.Module):
    ...
    def devoxelize_with_centroids(self, out: ME.SparseTensor, x: ME.TensorField, h_embs):
        out = self.final(torch.cat([out.interpolate(x).F, h_embs], dim=1))
        return out
