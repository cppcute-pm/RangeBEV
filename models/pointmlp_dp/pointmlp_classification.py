...
class PointMLP(Model):
    def __init__(self, cfgs, out_dim):
        super(PointMLP, self).__init__(points=cfgs.num_points,
                                        class_num=15,
        ...
        self.f_fc = nn.Sequential(
            nn.Conv1d(cfgs.embed_dim * 2**(self.f_layer), self.out_dim, kernel_size=1, stride=1, padding=0, bias=False),
            nn.BatchNorm1d(self.out_dim),
            )
        self.c_fc = nn.Sequential(
            nn.Conv1d(cfgs.embed_dim * 2**(self.c_layer), self.out_dim, kernel_size=1, stride=1, padding=0, bias=False),
            nn.BatchNorm1d(self.out_dim),
            )

    def forward(self, x):
        ### x: [B, N, 3]
        if x.shape[1] != self.points:
            fps_idx = pointnet2_utils.furthest_point_sample(x, self.points).long()  # [B, self.points]
            x = index_points(x, fps_idx)  # [B, self.points, 3]
        xyz = x.clone()
        batch_size, _, _ = x.shape
        x = self.embedding(x.permute(0, 2, 1))  # B,D,N
        for i in range(self.stages):
            # Give xyz[b, p, 3] and fea[b, p, d], return new_xyz[b, g, 3] and new_fea[b, g, k, d]
            xyz, x = self.local_grouper_list[i](xyz, x.permute(0, 2, 1))  # [b,g,3]  [b,g,k,d]
            x = self.pre_blocks_list[i](x)  # [b,d,g]
            x = self.pos_blocks_list[i](x)  # [b,d,g]
            if (i + 1) == self.f_layer:
                f_ebds = x # [B, C, n_points]
                f_points = xyz # [B, n_points, 3]
            elif (i + 1) == self.c_layer: 
                c_ebds = x  # [B, C, n_points]
                c_points = xyz # [B, n_points, 3]
                break
...
def pointMLP(num_classes=40, **kwargs) -> Model:
    return Model(points=16384, class_num=num_classes, embed_dim=64, groups=1, res_expansion=1.0,
