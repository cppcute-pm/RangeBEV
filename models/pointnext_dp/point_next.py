...
class SetAbstraction_v2(SetAbstraction):

    def forward(self, p, f, next_p):
        f = f.contiguous()
        new_p = next_p
        with torch.no_grad():
            new_p_p_dist = torch.cdist(new_p, p, p=2.0) # (B, M, N)
            idx = torch.argmin(new_p_p_dist, dim=-1, keepdim=False)
        """ DEBUG neighbor numbers. 
        query_xyz, support_xyz = new_p, p
        radius = self.grouper.radius
        dist = torch.cdist(query_xyz.cpu(), support_xyz.cpu())
...
@MODELS.register_module()
class PointNextEncoder_v1(PointNextEncoder):
    ...
    def __init__(self,
        ...
        super(PointNextEncoder_v1, self).__init__(in_channels=in_channels,
                                                         width=width,
                                                         blocks=blocks,
                                                         strides=strides,
                                                         block=block,
    ...
    def forward_seg_feat(self, p0, f0=None):
        ...
        for i in range(0, len(self.encoder)):
            ...
            if (i + 1) == self.c_layer:
                c_feats = f[-1]
                c_points = p[-1]
        return p, f, c_feats, f_feats, c_points, f_points

    def forward(self, p0, f0=None):
        return self.forward_seg_feat(p0, f0)

@MODELS.register_module()
class PointNextEncoder_v2(PointNextEncoder):
    ...
    def __init__(self,
        ...
        super(PointNextEncoder_v2, self).__init__(in_channels=in_channels,
                                                         width=width,
                                                         blocks=blocks,
                                                         strides=strides,
                                                         block=block,
...
@MODELS.register_module()
class PointNextEncoder_v3(nn.Module):
    r"""The Encoder for PointNext 
    `"PointNeXt: Revisiting PointNet++ with Improved Training and Scaling Strategies".
    <https://arxiv.org/abs/2206.04670>`_.
    .. note::
    ...
    def __init__(self,
                 channels_list: list = [32, 64, 128, 256, 512],
                 blocks: List[int] = [4, 7, 4, 4],
                 strides: List[int] = [4, 4, 4, 4],
                 block: str or Type[InvResMLP] = 'InvResMLP',
                 nsample: int or List[int] = 32,
                 radius: float or List[float] = 3.5,
                 aggr_args: dict = {'feature_type': 'dp_fj', "reduction": 'max'},
                 group_args: dict = {'NAME': 'ballquery'},
                 sa_layers: int = 1,
                 sa_use_res: bool = False,
    ...
    def _make_enc(self, block, in_channels, channels, blocks, stride, group_args, is_head=False):
        layers = []
        radii = group_args['radius']
        nsample = group_args['nsample']
        group_args['radius'] = radii[0]
        group_args['nsample'] = nsample[0]
        layers.append(SetAbstraction_v2(in_channels, channels,
    ...
    def forward_seg_feat(self, p_pre, f_pre):
        curr_f = f_pre[0]
        curr_p = p_pre[0]

        for i in range(0, len(self.encoder)):
            if i != 0:
                curr_f = torch.cat([curr_f, f_pre[i]], dim=1)
            curr_p, curr_f = self.encoder[i][0](curr_p, curr_f, p_pre[i+1])
            for j in range(1, len(self.encoder[i])):
                curr_p, curr_f = self.encoder[i][j]([curr_p, curr_f])
        return curr_p, curr_f

    def forward(self, p_pre, f_pre):
        return self.forward_seg_feat(p_pre, f_pre)
...
@MODELS.register_module()
class PointNextDecoder_modified1(PointNextDecoder):

    def forward(self, p, f):
        for i in range(-1, -len(self.decoder) - 1, -1):
            f[i - 1] = self.decoder[i][1:](
                [p[i], self.decoder[i][0]([p[i - 1], f[i - 1]], [p[i], f[i]])])[1]
            if self.out_layer_num == -i:
                return f[i - 1]
        return f[-len(self.decoder) - 1]

@MODELS.register_module()
class PointNextDecoder_v1(PointNextDecoder):
    ...
    def __init__(self,
                 ...
                 decoder_layers: int = 2, 
                 decoder_stages: int = 4,
                 blocks: int = [1, 2, 3, 2, 2],
                 **kwargs):
        super(PointNextDecoder_v1, self).__init__(encoder_channel_list, 
                                                  decoder_layers, 
                                                  decoder_stages,
                                                  **kwargs)
        self.f_layer = cfgs.f_layer
        self.c_layer = cfgs.c_layer
        self.encoder_length = len(blocks)
    ...
    def forward(self, p, f):
        ...
        for i in range(-1, -len(self.decoder) - 1, -1):
            f[i - 1] = self.decoder[i][1:](
                [p[i], self.decoder[i][0]([p[i - 1], f[i - 1]], [p[i], f[i]])])[1]
            if (self.c_layer - self.encoder_length) == (-i):
                c_feats = f[i - 1]
                c_points = p[i - 1]
            if (self.f_layer - self.encoder_length) == (-i):
                f_feats = f[i - 1]
                f_points = p[i - 1]
                return c_feats, f_feats, c_points, f_points

@MODELS.register_module()
class PointNextDecoder_v2(PointNextDecoder):
    ...
    def __init__(self,
                 ...
                 decoder_stages: int = 4, 
                 **kwargs):
        super(PointNextDecoder_v2, self).__init__(encoder_channel_list, 
                                                  decoder_layers, 
                                                  decoder_stages, 
                                                  **kwargs)
