...
class TransformerEncoderLayer(Module):
    ...
    def forward(self, src: torch.Tensor, *args, **kwargs) -> torch.Tensor:
        src_temp, attn_weight = self.self_attn(self.pre_norm(src))
        src = src + self.drop_path(src_temp)
        src = self.norm1(src)
...
class TransformerClassifier(Module):
    ...
    def forward(self, x):
        ...
        if self.seq_pool:
            global_token = torch.matmul(F.softmax(self.attention_pool(x), dim=1).transpose(-1, -2), x).squeeze(-2)
