"""
this range_projection function is from https://github.com/PRBonn/range-mcl
...
def range_projection_v2(current_vertex, fov_up=3.0, fov_down=-25.0, proj_H=64, proj_W=900, max_range=50):
  """ Project a pointcloud into a spherical projection, range image.
    Args:
      current_vertex: raw point clouds
    Returns:
  ...
  valid_depth_idx_in_original = np.nonzero((depth > 0) & (depth < max_range))[0]
  depth = depth[(depth > 0) & (depth < max_range)]
  ...
  real_indices = valid_depth_idx_in_original[indices]
  
  proj_range[proj_y, proj_x] = depth
  proj_idx[proj_y, proj_x] = real_indices
