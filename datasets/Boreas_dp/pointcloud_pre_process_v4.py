...
def process_sequence(seq_list):
    ...
    for lidar_idx in lidar_idxs:
        ...
        save_path_cliped_fov_mask = os.path.join(target_seq_lidar_path, filename + "cliped_fov_mask.npy")
        save_path_all = os.path.join(target_seq_lidar_path, filename + "_all.npy")
        ...
        pc_reflection = curr_lidar_frame.points[:, :4].astype(np.float32) # (N, 4)
        pc = pc_reflection[:, :3]
        ...
        pc_clip_fov_mask = mask_1 & mask_2 # (N,)

        # vis_pc_clip_fov = pc[pc_clip_fov_mask]

        np.save(save_path_cliped_fov_mask, pc_clip_fov_mask)
        np.save(save_path_all, pc_reflection)
...
if __name__ == "__main__":
    ...
    target_name = "Boreas_minuse_cliped_fov_and_reflection"
