...
from .openpcseg_utils import initial_voxelize, voxel_to_point
import os
    ...
    def forward(self, batch_dict, return_logit=False, return_tta=False):
        ...
        if self.training:
            ...
            for idx in range(int(invs.C[:, -1].max()) + 1):
                ...
                point_predict.append(outputs_mapped[:batch_dict['num_points'][idx]])
                point_predict_logits.append(outputs_mapped_logits[:batch_dict['num_points'][idx]])

            return {'point_predict': point_predict, 'point_predict_logits': point_predict_logits}
