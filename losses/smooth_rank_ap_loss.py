...
def step_rank(tens, tau, rho, offset=None, delta=None, start=0.5, target=None, general=False):
    ...
    tens[target] = torch.where(tens[target] == 0, torch.tensor([1.0], dtype=tens.dtype, device=tens.device), torch.tensor([0.0], dtype=tens.dtype, device=tens.device))
...
class SmoothRankAP(nn.Module):
    def __init__(
        ...
        self.return_type = cfgs.return_type
        assert cfgs.return_type in ["1-mAP", "1-AP", "AP", 'mAP']
...
class HeavisideAP(SmoothRankAP):
    ...
    def __init__(self, cfgs):
        ...
        super().__init__(rank_approximation, cfgs)
...
class SmoothAP(SmoothRankAP):
    ...
    def __init__(self, cfgs):
        rank_approximation = partial(tau_sigmoid, tau=cfgs.tau)
        super().__init__(rank_approximation, cfgs)
        self.tau = cfgs.tau
...
class SupAP(SmoothRankAP):
    ...
    def __init__(self, cfgs):
        rank_approximation = partial(step_rank, tau=cfgs.tau, rho=cfgs.rho, offset=cfgs.offset, delta=cfgs.delta, start=cfgs.start)
        super().__init__(rank_approximation, cfgs)
        self.tau = cfgs.tau
        self.rho = cfgs.rho
